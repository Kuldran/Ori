    ...
    */
    ...
    public function Log($key = NULL, $value = NULL, $log_level = self::SEVERITY_TRACE) {
        if ($this->CanLog($log_level)) {
            // Basic Logging
            if (file_put_contents($this->cwd.self::LOG_BASE_PATH.$this->file, $this->Format($key, $value, $log_level), FILE_APPEND | LOCK_EX) === FALSE) {
            ...
            if ($log_level <= self::SEVERITY_WARNING) {
                if (file_put_contents($this->cwd.self::LOG_BASE_PATH.$this->generic_log_file, $this->Format($key, $value, $log_level), FILE_APPEND | LOCK_EX) === FALSE) {
                    throw new Exception('Can not write to log file. Please check the path and that the folder is writeable.');
                }
    ...
    public function Warning($key = NULL, $value = NULL) {
        return $this->Log($key, $value, static::SEVERITY_WARNING);
    }
