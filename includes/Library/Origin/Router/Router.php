...
class Router extends \Origin\Utilities\Types\Singleton {
    ...
    */
    ...
    public function __construct() {
        $settings = Settings::Get('routes')->Values([]);
        if (!$settings->offsetExists('routes')) {
            throw new Exception('You must have routes defined in your routes file.');
        }

        $this->routes = $settings->offsetGet('routes');
        if ($settings->offsetExists('modules')) {
            $this->modules = $settings->offsetGet('modules');
        }
    ...
    */
    private function PathFinder($path, $route, array $variables = []) {
            ...
            if ($this->GetFile($path)) {
                ...
                if ($this->modules !== NULL && $this->modules instanceof Hash) {
                    foreach ($this->modules as $module) {
                        if (call_user_func_array([(new $module), 'Precall'], [$class, $route, $variables, $_REQUEST]) === FALSE) {
                            throw new Exception(sprintf('Unable to load module %s or module is invalid.', $module));
                        }
                    }
                }
                
                if (call_user_func_array([(new $class), $this->GetMethod($path)], $variables) === FALSE) {
